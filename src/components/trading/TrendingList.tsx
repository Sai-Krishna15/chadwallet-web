...
export function TrendingList({ 
  ...
  return (
    <div className="bg-card border border-white/5 rounded-2xl p-4 flex flex-col h-full min-h-[600px]">
      ...
      <div className="flex gap-2 mb-4 bg-background/50 p-1 rounded-lg">
        ...
        {list.length === 0 ? (
          ...
          list.map((token, i) => {
            ...
            return (
              <Link 
                key={`${token.address}-${i}`} 
                href={`/trade/${encodeURIComponent(token.symbol)}`}
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex flex-col">
                    ...
                    <span className="text-xs text-muted-foreground max-w-[80px] truncate">{token.name}</span>
                  </div>
                </div>
